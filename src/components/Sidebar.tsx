...
import {
  LayoutDashboard, ShieldCheck, ScrollText, ClipboardList,
  BarChart3, BotMessageSquare, AlertTriangle, Map, Route,
} from 'lucide-react';
...
const nav = [
  ...
  { href: '/risk', label: 'Risk Classification', icon: AlertTriangle },
          ...
          return (
            <Link key={href} href={href}
              className={`flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-colors ${
                active ? 'bg-fuchsia-600 text-white' : 'text-purple-200 hover:bg-white/10 hover:text-white'
              }`}
            >
              <Icon size={17} />
              {label}
